...
3. 使用 powermetrics 獲取系統溫度（需要 sudo）
...
class MacHardwareMonitor:
    ...
    def get_temperature_info(self):
        ...
        技術說明：
        - 使用 powermetrics 命令獲取溫度資訊
        - 需要管理員權限（sudo）
        - 設置 5 秒超時避免懸掛
        
        實現步驟：
        1. 執行 powermetrics 命令
        2. 解析輸出尋找溫度資訊
        3. 提取數值並格式化
        
        返回資料結構：
        {
            'cpu_temp': str  # CPU 溫度（例：'45.3°C'）
        }
        或
        {
            'note': str     # 錯誤說明
        ...
        """
        try:
            # 使用 powermetrics 獲取溫度資訊（需要 sudo 權限）
            result = subprocess.run(
            ...
            if result.returncode == 0:
                ...
                temp_lines = [line for line in lines if 'CPU die temperature' in line]
                if temp_lines:
                    return {'cpu_temp': temp_lines[0].split(':')[1].strip()}
        except:
            pass
        return {'note': '需要管理員權限才能讀取溫度'}
...
def main():
    ...
    if choice == "1":
        ...
        print("Temperature:", json.dumps(monitor.get_temperature_info(), indent=2, ensure_ascii=False))
